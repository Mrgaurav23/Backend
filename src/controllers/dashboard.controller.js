...
const getChannelStats = asyncHandler(async (req, res) => {
  ...
  if (!stats.length) {
    // If stats array is empty, it means no videos or subscribers were found.
    // We still return 200 with zero counts.
  ...
  // Return the response (send the first/only element of the stats array)
  return res
    .status(200)
    .json(
...
const getChannelVideos = asyncHandler(async (req, res) => {
  // Get Channel ID and Pagination Parameters
  ...
  if (!channelId) {
    ...
    throw new ApiError(401, "Channel ID not found. User is not authenticated.");
  }
  ...
  const pipeline = [
    ...
    {
      // Final projection to select necessary fields
      $project: {
        videoFile: 1,
        thumbnail: 1,
        title: 1,
  ...
  // Set up aggregation and pagination
