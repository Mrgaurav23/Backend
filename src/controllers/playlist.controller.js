...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
    ...
    if(!updatedplaylist){
        throw new ApiError(500,"error while remove video from the playlist")
    }
...
const deletePlaylist = asyncHandler(async (req, res) => {
    ...
    //  Authorization Check (Crucial Security Check)
    if (playlist.owner.toString() !== userId.toString()) {
...
const updatePlaylist = asyncHandler(async (req, res) => {
    ...
    if (!name && !description) {
        throw new ApiError(400, "Please provide either name or description to update");
    }
    ...
    if (!updatedPlaylist) {
        throw new ApiError(500, "Failed to update playlist in database");
    }
